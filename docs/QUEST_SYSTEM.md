...
`QuestType` は YAML から `SimpleQuestLoader` → `QuestConverter` を介してロードされ、`DomainQuestTypes` レジストリに登録される不変データオブジェクト。

### データモデル

```kotlin
// domain/quest/model/QuestType.kt
...
- `loadFrom()` で DB から復元、`snapshot()` で保存用スナップショット取得

## Actions（アクションシステム）

`Actions` セクションで定義。`BukkitActionDispatcher` が実行を担当。詳細は **[wiki/quest/ACTIONS.md](../wiki/quest/ACTIONS.md)** を参照。
...
| `PvELevel` | `PVELEVEL_EXP` | PvELevel 経験値付与 | ⚠️ 未実装 |
