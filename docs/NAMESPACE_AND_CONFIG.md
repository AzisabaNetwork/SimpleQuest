...
LifeQuest はすべてのオブジェクト (QuestType, QuestCategory, Stage) を `namespace:key` の**完全修飾名 (FQN)** で管理する。
...
1. `plugins/LifeQuest/` 直下の `@^([a-zA-Z0-9_-]+)$` にマッチするディレクトリを列挙
...
```kotlin
// dir  = plugins/LifeQuest/@mynamespace/types
// file = plugins/LifeQuest/@mynamespace/types/event/boss.yml
// result = event/boss
...
`/lifequest reload` または Loader.load() の実行時に以下の処理が行われる:
