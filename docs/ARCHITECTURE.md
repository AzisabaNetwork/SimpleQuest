...
**LifeQuest** は [アジ鯖](https://www.azisaba.net) の Reincarnation PvE サーバー向けに開発された Paper プラグインのクエストマネージャーです。
...
```
net.azisaba.lifequest
├── LifeQuest.kt               — プラグインエントリポイント
├── LifeQuestBootstrap.kt      — PluginBootstrap (ライフサイクル)
├── LifeQuestLoader.kt         — YAML 読み込み → レジストリ / DB 登録
│
├── quest/                     — クエストドメイン
│   ├── Quest.kt
│   ├── QuestType.kt
...
1. **PluginBootstrap** (LifeQuestBootstrap)
   - LifecycleEvents.COMMANDS 購読
   - コマンド登録

2. **onEnable** (LifeQuest)
   - プラグインディレクトリ作成
   - `config.yml` 読み込み
