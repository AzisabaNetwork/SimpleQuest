...
Kunectron ベースの GUI に加えて、**Lore ベースの詳細表示**を導入。
...
| GUI 名 | 種類 | 用途 |
|---|---|---|
| QuestGui | ChestGui | クエスト選択画面 (カテゴリ別、ページング、検索) |
| QuestMenuGui | ChestGui | クエスト進行中のメニュー |
| QuestDetailGui | 2x2 クラフト枠 | クエスト詳細表示 (Lore) |
| QuestPanelGui | ScoreboardGui | クエスト進行中のスコアボード |
| PartyMenuGui | ChestGui | パーティ管理画面 |
| PartyCreateGui | ChestGui | パーティ作成画面 |
| PartyInviteGui | ChestGui | 招待プレイヤー選択画面 |
| ConfirmGui | ChestGui | 確認ダイアログ |
| SearchGui | SignGui | 検索用看板入力 |
...
1. プレイヤーがパーティリーダーか
2. パーティが既にクエスト中でないか
3. パーティがサイズ条件を満たしているか (`minPlayers` / `maxPlayers`)
4. 全メンバーが解放済みか (`player_quest_types` テーブル)
5. 全メンバーが受注条件を満たしているか (PvELevel / Money / PartyMode)
